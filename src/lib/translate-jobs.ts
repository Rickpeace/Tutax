 ...
 *
 * Autorisierung: Dieses Modul ist BEWUSST keine Server-Action-Datei ("server-only") — die
 * Funktionen laufen mit dem Admin-Client (after()/Backfill ohne RLS-Kontext) und prüfen
 * KEINE Rechte. Sie dürfen daher nur von Server-Code aufgerufen werden, der vorher selbst
 * autorisiert hat. Der Builder-Knopf geht über die Action in app/app/actions-translate.ts
 * (prüft Rolle + Konto). Vorher war das hier eine "use server"-Datei: jede Funktion war
 * ohne Anmeldung/Rechteprüfung aus dem Browser aufrufbar (KI-Kosten auf fremde Konten).
 */
